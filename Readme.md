...
Geforce GTX1080 (VRAM 8GB) での動作を確認しています。最低でも VRAM 8GB は必要です (VRAM 12GB のグラボがほしい…) 。
...
- **03-Segments:** `create-segments` サブコマンドでセグメント化された音声ファイルが入るディレクトリ
  - `create-segments` サブコマンドを実行すると、`02-PreparedSources` にある音声ファイルが書き起こし文や無音区間などをもとに大まかに一文ごとに分割され、このディレクトリに保存されます。
...
ここでは、作成する TTS モデルの名前を「MySpeaker1」「MySpeaker2」とします。
...
実行すると、`01-Sources` 以下にある音声ファイルの BGM や雑音が除去され、`02-PreparedSources` 以下に書き起こしデータとともに保存されます。  
`02-PreparedSources` 以下にすでに当該音声の下処理済みの音声ファイルや書き起こしデータが存在する場合は、そのファイルが再利用されます。

さらに、`02-PreparedSources` 以下に保存した音声ファイルが書き起こし文や無音区間などをもとに大まかに一文ごとに分割され、`03-Segments` 以下に保存されます。  
...
Aivis のデータセット作成工程は大半が `create-segments` サブコマンドで自動化されています。  
...
この Web UI から `03-Segments` 以下の一文ごとにセグメント化された音声と書き起こし文をもとにアノテーションを行い、手動でデータセットを作成できます。

`create-datasets` サブコマンドの第一引数二は、`03-Segments` 以下に自動生成されている、セグメント化された音声ファイルのディレクトリ名を指定します。通常は `01-Sources` 以下の音声ファイル名と同じです。  
内部では Glob が使われているため、ワイルドカード (`*`) を使い複数のディレクトリのアノテーション作業を一括で行えます。
...
VRAM 不足で実行途中に CUDA Out Of Memory エラーが出る場合は、`--batch-size` で学習時のバッチサイズを小さくしてください。Geforce GTX1080 ではバッチサイズ 3 〜 4 でギリギリな印象です。
